...
//        Aff_valeur(convert_Hex_Dec(count),BIT3);
        count++;
    ...
    while(1)
    {
      ...
      if(!(P2IN&BIT0) && !(P2IN&BIT1)) // Si les boutons SW sont activé
      {
      ...
      __delay_cycles(500000); // Attente 1 seconde
      ...
      val = ADC_Lire_resultat();        //Get the value of ADC
      U = val * 2.5 /1023 *1000;       //Calculate with the 1024 steps of ADC10, the result is mV
      count++;
//          if(count>2){
          /* Particles sensor */
          send_string(commande);
...
//          for (j=0; j<5000; j++);
          ...
          // Aff_valeur(convert_Hex_Dec(1998),BIT3);
//          }
      // Aff_valeur(convert_Hex_Dec((unsigned int)U), BIT4);   //Change to a form which 7-segments can know.
      // Aff_valeur(convert_Hex_Dec(1998), BIT2);   //Change to a form which 7-segments can know.
    }
}
