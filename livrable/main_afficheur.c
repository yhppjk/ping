...
/* int get_opacite()                                                                 */
/*...................................................................................*/
/* Description : get the value of opacity from the UART                              */
/* Inputs :                                                                          */
/*      - void                                                                       */
/*                                                                                   */
/* Output : int : the value of opacity                                               */
/*...................................................................................*/
int get_opacite();
...
/* int get_temperature()                                                             */
/*...................................................................................*/
/* Description : get the value of temperature from the UART                          */
/* Inputs :                                                                          */
/*      - void                                                                       */
/*                                                                                   */
/* Output : int : the value of opacity                                               */
...
/* int getpm25()                                                                     */
/*...................................................................................*/
/* Description : get the value of pm2.5 from the UART                                */
/* Inputs :                                                                          */
/*      - void                                                                       */
/*                                                                                   */
/* Output : int : the value of opacity                                               */
...
int main(void)
{
  ...
  P1OUT |= BIT0;
  __delay_cycles(1000000); // Attente 1 seconde
  ...
  {
    P1OUT |= BIT0;
    __delay_cycles(100000); // Attente 100 millisecondes pour être sûr qu'il est lu par le SE (cadencement 100 ms sur SE)

    // if(!(P1IN&BIT3)) // Détection appui bouton poussoir
    if((read[0] == 0XFE))
    {
      flag_cligno = 1;
    }

    if((flag_cligno == 1) && (count < 50)) // clignottment pendant 10 sec
    {
      // Extinction des écrans pour clignottement
      Aff_Efface(BIT0); // à modifier (lorsque l'électrovanne est activé)
      Aff_Efface(BIT1);
      Aff_Efface(BIT2);
      count++;
    }

    __delay_cycles(100000); // Attente 100 millisecondes
    __enable_interrupt();


    Aff_valeur(convert_Hex_Dec(get_temperature()),BIT0); // Acquisition température et affichage sur afficheur 7 segments en SPI (LCD1)
    Aff_valeur(convert_Hex_Dec(get_opacite()),BIT1); // Acquisition opacité et affichage sur afficheur 7 segments en SPI (LCD2)
    Aff_valeur(convert_Hex_Dec(getpm25()),BIT2); // Acquisition taux de particules fines et affichage sur afficheur 7 segments en SPI (LCD3)

    P1OUT &= ~BIT0;
    __delay_cycles(100000); // Attente 1 seconde
  }
...
__interrupt void USCI0RX_ISR(void)
{
    if (IFG2 & UCA0RXIFG){
     ...
     read[i]=UCA0RXBUF;
     i++;
     if((read[0] != 0xFF) && (read[0] != 0xFE)) // Si le premier octet est différent de 0xFF ou 0xFE, alors on n'écoute pas
     {
        i=0;
     }
...
int get_temperature(){
    return (int)read[1]*16*16+(int) read[2];
}


int get_opacite(){
    return (int)read[3]*16*16+(int) read[4];
}


int getpm25(void){
    return (int)read[5]*16*16+(int) read[6];
}
